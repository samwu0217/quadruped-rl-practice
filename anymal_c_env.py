...
class AnymalCMujocoEnv(MujocoEnv):
    ...
    def __init__(self, ctrl_type="torque", **kwargs):
        model_path = Path(f"./anybotics_anymal_c/scene.xml")
        MujocoEnv.__init__(
            self,
    ...
    def linear_velocity_tracking_reward(self):
        ...
        vel_sqr_error = np.square(self._desired_velocity[0] - self.data.qvel[0])
        reward_x = np.exp(-vel_sqr_error / 0.02)
